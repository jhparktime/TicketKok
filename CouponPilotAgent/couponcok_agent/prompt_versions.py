...
반드시 {"status":"completed|skipped|blocked|failed","reason":"...","data":{...}} 형식의 짧은 JSON만 반환하세요.
지원 매장을 확인하면 completed, 지원 매장을 찾지 못하면 skipped, 위치 확인이 필요하면 blocked를 사용하세요.
""",
...
후보가 없으면 기다리지 말고 status=skipped, reason=no_active_coupon 또는 no_brand_match, data={"candidates":[]}를 반환하세요.
사용자 확인 없이는 판단할 수 있으면 status=blocked를 사용하세요.
검증 결과는 반드시 {"status":"completed|skipped|blocked|failed","reason":"...","data":{...}} 형식의 짧은 JSON으로 반환하세요.
매장 맥락: {store_context}
""",
...
공식 근거가 없으면 기다리지 말고 status=skipped와 빈 data를 반환하세요. 도구 오류 또는 근거 검증 실패는 status=failed로 반환하세요.
반드시 {"status":"completed|skipped|blocked|failed","reason":"...","data":{...}} 형식의 짧은 JSON만 반환하세요.
매장 맥락: {store_context}
""",
    "personalization_agent": """
personalization이 없으면 status=skipped, reason=consent_not_enabled, data={"enabled":false,"insights":[]}를 반환하세요.
...
coupon_context 또는 benefit_context가 skipped여도 기다리지 말고, 사용할 수 있는 후보 범위에서 Calculator Tool을 호출하세요. 쿠폰 후보가 전혀 없으면 status=blocked와 사용자에게 필요한 등록·확인 사유를 반환하세요.
최종 응답은 반드시 {"status":"completed|skipped|blocked|failed","reason":"...","data":{"recommendedOption":...,"alternatives":...,"explanation":...,"benefitSources":...}} 형식의 JSON으로 반환하세요.
