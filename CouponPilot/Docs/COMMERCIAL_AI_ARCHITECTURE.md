...
| 4. 사용 패턴 해석 | Personalization Agent + 서버 정책 | 동의된 브랜드별 사용 횟수·간격과 만료 위험을 해석하고, 서버 정책이 개인화 표시 우선순위를 산정 | 원본 구매 이력 접근, 민감 속성 추론, 금액·절약액·가격 기준 순위 변경 | 개인화 문구 없이 Calculator 가격 비교 유지 |
